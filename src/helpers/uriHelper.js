export const vatsimAuthRedirectUrl = `${getAuthEndpoint()}/oauth/authorize?client_id=${getClientId()}&redirect_uri=${
  ...
  export const discordRedirectUrl = `https://discord.com/api/oauth2/authorize?client_id=1080125042400182352&redirect_uri=https%3A%2F%2Fwww.zfwartcc.net%2Flogin%2Fdiscord&response_type=code&scope=identify`;
  
  function getClientId() {
    const betaClientId = 1225;
    const prodClientId = 1226;
    const devClientId = 466;
  
    if (window.location.origin.includes("staging")) {
      return betaClientId;
    } else if (window.location.origin.includes("zfwartcc.net")) {
      return prodClientId;
    } else {
      return devClientId;
    }
